...
const render=()=>{if(!chartRef.value)return;if(!chart)chart=echarts.init(chartRef.value)
  chart.setOption({tooltip:buildTooltip((params)=>{const item=props.items[params.dataIndex];return `<strong>${item.name}</strong><br/>拥有者 ${item.ownerCount} 人<br/>覆盖率 ${item.coverage}%<br/>${item.role}`}),
    grid:{left:110,right:36,top:30,bottom:34},xAxis:{type:'value',max:12,name:'人数',axisLabel:{color:chartPalette.muted},splitLine},
    ...
    series:[{type:'bar',barMaxWidth:18,data:[...props.items].reverse().map((item)=>({value:item.ownerCount,itemStyle:{color:item.role==='最终候选'?chartPalette.gold:chartPalette.accent,opacity:item.excluded?.18:1,borderRadius:[0,4,4,0]}})),
      label:{show:true,position:'right',color:chartPalette.text,fontSize:10},markLine:{symbol:'none',label:{formatter:'目标 8 人',color:chartPalette.gold,position:'end'},lineStyle:{color:chartPalette.gold,type:'dashed'},data:[{xAxis:8}]}}]},true)
...
<style scoped>
.section-kicker{color:var(--subtle);font-size:.7rem;font-weight:800}.visible-subtitle{display:block!important;margin:5px 0 0;color:var(--muted);font-size:.76rem}.coverage-chart{height:750px}
</style>
